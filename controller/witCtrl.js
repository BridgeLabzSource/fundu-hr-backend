...
const message = (msg, expValue, cb) => {
    ...
    client.message(msg, {})
        .then((data) => {
            ...
            datetime ?
                cb(null, { "time": moment(datetime).format("YYYY-MM-DD HH:mm:ss Z"), "on_off": on_off }) : cb(null, {"datat":"dkdsakd", "time": moment().utcOffset("+05:30").format("YYYY-MM-DD HH:mm:ss Z"), "on_off": on_off }): cb({ "msg": data._text, "error": "in entities on_off not found" }, null): cb({ "msg": data._text, "error": "in entities intent not found" }, null)
        })
        .catch((err) => {
