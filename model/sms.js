            ...
            if (existingUser) {
                ...
                request({ url: url, method: 'POST', json: true, headers: { 'Authorization': auth, 'Content-Type': 'application/json' } }, function(ee, r, body) {
                    var data = r.body,
                        ...
                        datetime= moment().format("YYYY-MM-DD HH:mm:ss Asia/Kolkata Z");
                    ...
                    if (intent == 'Work' || intent == 'office' && on_off == 'on') {
                        ...
                        var diff = moment.utc(moment(datetime, "YYYY-MM-DD HH:mm:ss Z").diff(moment(existingUser.inTime, "YYYY-MM-DD HH:mm:ss Z"))).format("HH:mm:ss");
                        var result = {
